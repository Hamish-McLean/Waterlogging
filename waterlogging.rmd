...
e1_diameter_model <- lmer(
  diameter ~ duration + timepoint + duration:timepoint + (1 | block) + (1 | tree), # + (timepoint | tree),
...
e1_height_model <- lmer(
  height ~ duration + timepoint + duration:timepoint + (1 | block) + (1 | tree), # + (timepoint | tree),
...
e2_canker_model <- glmmTMB(
  cbind(rasp_cankers, rasp_inoculations - rasp_cankers) ~ duration + (1 | block) + (1 | tree),
  family = betabinomial(link = "logit"),
