...
growth_rate <- function(data, by){
  # Filter data for T1 and T3 timepoints
  data_subset <- data[timepoint %in% c("T1", "T3")]

  # Remove height data for broken trees
  broken <- unique(data_subset[health %in% c("top_broken", "top_dead"), tree])
  data_subset[tree %in% broken, height := NA]
  ...
  height_growth <- data_subset[
    ...
    .(
      ...
      final_height = height[timepoint == "T3"]
    ),
    by = by
  ][, `:=`(
  ...
  diameter_growth <- data_subset[
    ...
    .(
      ...
      final_diameter = diameter[timepoint == "T3"]
    ),
    by = by
  ][, `:=`(
...
# e3_growth <- growth_rate(e3, "pot")
# e4_growth <- growth_rate(e4, "pot")
