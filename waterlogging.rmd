...
merge_cols <- c("diameter", "heaght", "health")
...
e3 <- Reduce(
  ...
  list(e3_2024_10_growth, e3_2025_05_cankers, e3_2025_09_cankers)
)[, experiment := "E3"]
...
plot_tree_health(
  e3_health_summary[timepoint == "T1" & (health == "healthy" | health == "dead")], 
  "treatment", "genotype"
)
ggsave(
  filename = "tree_health_e3.png", path = figures_dir,
...
emm_diameter_e2 <- emmeans(e2_diameter_model, trt.vs.ctrl ~ duration | timepoint)
...
e2_height_model <- lmer(
  ...
  data = e2[!(health %in% top_removals)]
)
...
emmeans(e3_leaf_scar_canker_model, ~ genotype | treatment)
