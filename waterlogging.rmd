...
Count of dead and 'healthy' (not dead) trees summarised per plot (or per subplot
for experiment 3).
...
model <- glm(
  ...
  data   = mortalityByPlot(e1[duration != "8"], "T3"),
...
e1_mort_t3_emm <- emmeans(model, ~ duration, type = "response") |> as.data.frame()
...
e2_mort_t3_emm <- emmeans(model, ~ duration, type = "response") |> as.data.frame()
...
model <- glmmTMB(design, dispformula = ~ genotype, data = e3_mortality, family = betabinomial(link = "logit"))
# check_model(model) # High collinearity
# sim <- simulate_residuals(model)
...
# Per genotype models
design <- cbind(dead, total - dead) ~ block + treatment
model <- glm(
  design, data = e3_mortality[genotype == "MM106"],
...
e3_mort_MM106_emm <- emmeans(model, ~ treatment, type = "response") |> as.data.frame()

model <- glm(
  design, data = e3_mortality[genotype == "M9"],
  family = binomial(link = "logit"), method = brglmFit
)
check_model(model) # Reasonable
...
anova(update(model, . ~ block), model, test = "Chisq")
emmeans(model, trt.vs.ctrl ~ treatment, type = "response")
e3_mort_M9_emm <- emmeans(model, ~ treatment, type = "response") |> as.data.frame()

e3_mort_emm <- bind_rows(
  setDT(e3_mort_MM106_emm)[, genotype := "MM106"],
  setDT(e3_mort_M9_emm)[, genotype := "M9"]
)
...
mortalityPlot <- function(data, timepoint = "T2") {
  ggerrorplot(
    data = mortalityByPlot(data, timepoint = "T2")[, mortality := dead / total],
    x = "duration",
    y = "mortality",
    add = "jitter",
    desc_stat = "mean_ci"
...
# emmeans error plot
emmMortalityPlot <- function(data, x, emm, facet = NULL, xlab = "Waterlogging treatment") {
  p <- ggplot() +
    geom_jitter(
      data = data,
      aes(x = get(x), y = mortality),# colour = block),
...
# E1 E2 combined
e1_e2_mortality[, experiment := gsub("E", "Experiment ", experiment)]
e1_e2_mortality_emm[, experiment := gsub("E", "Experiment ", experiment)]

a <- emmMortalityPlot(e1_e2_mortality, "duration", e1_e2_mortality_emm, "experiment", xlab = "Waterlogging duration (weeks)")
a
...
b <- emmMortalityPlot(e3_mortality, "treatment", e3_mort_emm, "genotype")
b

# ggboxplot(mortalityByPlotE3(e3), "treatment", "mortality", facet.by = "genotype", add = "jitter") +
#   scale_y_continuous(labels = label_percent(accuracy = 1), limits = c(0, 1))

# E1 and E2 at T3 (after second waterlogging)
e1t3 <- emmMortalityPlot(mortalityByPlot(e1[duration != "8"]), "duration", e1_mort_t3_emm, xlab = "Waterlogging duration (weeks)")

e2t3 <- emmMortalityPlot(mortalityByPlot(e2), "duration", e2_mort_t3_emm, xlab = "Waterlogging duration (weeks)")

ggarrange(e1t3, e2t3, labels = c("A", "B"), ncol = 2)
...
ggsave(filename = "mortality_combined_2.png", path = figures_dir, width = 14, height = 20, units = "cm")


# All combined

e1e1_t3 <- ggarrange(e1t3, e2t3, labels = c("B", "C"), ncol = 2)

ggarrange(a, e1e1_t3, b, c, labels = c("A", "", "D", "E"), ncol = 1)

ggsave(filename = "mortality_combined_3.png", path = figures_dir, width = 14, height = 20, units = "cm")
...
#'
summarise_growth <- function(data, measurement, trt_var, remove_health = NULL) {
  # Check columns
  if (!all(c(
    "timepoint", trt_var, measurement
  ) %in% colnames(data))) {
  ...
  data[
    ...
    by = c("timepoint", "plot", trt_var)
  ]#[, get(trt_var) := factor(get(trt_var))]
}

#' @title Plot growth data
#'
plot_growth <- function(data, measurement, treatment) {
...
plot_growth(summarise_growth(e2, "height", "duration"), "height", "duration")
...
pivotGrowth <- function(data, pivot, values = c("diameter", "height")) {
  dcast(data, pivot, value.var = values)[, experiment := data$experiment[[1]]]
}

#' Remove negative growth measurements (likely measurement error)
clean_growth <- function(dt) {
  # Metrics can be diameter or height (or extension_mean in E4)
  metrics <- unique(sub("_T[0-9]$", "", grep("_T[0-9]$", names(dt), value = TRUE)))
  for (m in metrics) {
    t1 <- paste0(m, "_T1"); t2 <- paste0(m, "_T2"); t3 <- paste0(m, "_T3")

    # If T2 < T1, set T2 to NA
    if (all(c(t1, t2) %in% names(dt))) dt[get(t2) < get(t1), (t2) := NA]

    # If T3 < T2, set T3 to NA
    if (all(c(t2, t3) %in% names(dt))) dt[get(t3) < get(t2), (t3) := NA]

    # If T3 < T1, set T3 to NA (ensuring non-decreasing from baseline)
    if (all(c(t1, t3) %in% names(dt))) dt[get(t3) < get(t1), (t3) := NA]
  }
  dt
}
...
e1_growth <- clean_growth(e1_growth)
# e1_growth[, diameter_T1c := scale(diameter_T1, center = TRUE, scale = FALSE)]
...
# anova(model, update(model, . ~ . + diameter_T1:duration))
emtrends(model, ~ duration, var = "diameter_T1")
...
e2_growth <- clean_growth(e2_growth)
...
e3_growth <- clean_growth(e3_growth)

design <- diameter_T2 ~ (diameter_T1 + treatment) * genotype + (1 | block / genotype / plot)
...
emmeans(model, revpairwise ~ genotype, type = "response")
...
e4_growth <- pivotGrowth(e4, pivot, values = c("diameter", "extension_mean"))
e4_growth <- clean_growth(e4_growth)

# Covariate approach
design <- diameter_T2 ~ diameter_T1 + treatment + (1 | block / plot)
...
e4_dia_emm <- emmeans(model, ~ treatment) |> as.data.table()
...
# emmeans(model, trt.vs.ctrl ~ treatment | genotype)
emmeans(model, revpairwise ~ genotype, type = "response")
e3_height_emm <- emmeans(model, ~ treatment | genotype) |> as.data.table()


# E4
# Extension rather than height

e4_ext <- e4[!is.na(extension_mean)]

design <- extension_mean ~ treatment + (1 | block / plot)
model <- lmer(design, e4_ext)
summary(model)
ranova(model)
anova(model, ddf = "Kenward-Roger", type = 3)
# check_model(model) # fine
emmeans(model, trt.vs.ctrl ~ treatment)
e4_ext_emm <- emmeans(model, ~ treatment) |> as.data.table()
...
growthByPlot <- function(data, t1, t2, x) {
  cols <- c(x, "block", "plot", if ("genotype" %in% colnames(data)) "genotype")
  data[!is.na(get(t2))][, .(T1 = mean(get(t1)), T2 = mean(get(t2))), cols]
}

# growthByPlot(e1_growth, "diameter_T1", "diameter_T2", "duration")


growthSlopePlot <- function(
  data, type, x, emm, final = "T2", facet = NULL, xlab = "Waterlogging treatment"
) {
  T1 <- paste0(type, "_T1")
  ...
  p <- ggplot() +
    geom_segment(
      data = dat,
      aes(x = x_T1, xend = x_T2, y = T1, yend = T2),# colour = block),
      size = 0.4, alpha = 0.2
    ) +
    ...
    geom_pointrange(
      data = emm,
      aes(x = x, y = emmean, ymin = lower.CL, ymax = upper.CL),
      size = 0.5
    ) +
    scale_x_continuous(
    ...
    labs(
      x = xlab,
      y = if (type == "diameter") "Diameter (mm)" else if (type == "height") "Height (cm)"
    ) +
  ...
  # E4 plot angle text
  # if (data$treatment[[1]] == "4 weeks winter") {
  if (data$experiment[[1]] == "E4") {
    p <- p + theme(axis.text.x = element_text(angle = 10, vjust = 1, hjust = 1))
  }
...
extPlot <- function(data, x, emm) {
  ggplot() +
  geom_jitter(
    data = data[, .(ext = mean(extension_mean)), c(x, "block", "plot")],
    aes(x = get(x), y = ext),# colour = block),
    width = 0.3, height = 0, alpha = 0.2, size = 2
  ) +
  geom_pointrange(
    data = emm,
    aes(x = get(x), y = emmean, ymin = lower.CL, ymax = upper.CL),
    # fatten = 1.8, 
    size = 0.5
  ) +
  labs(x = "Waterlogging treatment", y = "Extension (cm)") +
  guides(colour = "none") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 10, vjust = 1, hjust = 1))
}
...
# Diameter
# E1
dia_e1_t2_plot <- growthSlopePlot(e1_growth, "diameter", "duration", e1_dia_T2_emm, xlab = "Waterlogging duration (weeks)")
# ggsave("diameter_slope_E1.png", path = figures_dir)
dia_e1_t3_plot <- growthSlopePlot(e1_growth, "diameter", "duration", e1_dia_T3_emm, "T3", xlab = "Waterlogging duration (weeks)")

# E2
dia_e2_t2_plot <- growthSlopePlot(e2_growth, "diameter", "duration", e2_dia_T2_emm, xlab = "Waterlogging duration (weeks)")
dia_e2_t3_plot <- growthSlopePlot(e2_growth, "diameter", "duration", e2_dia_T3_emm, "T3", xlab = "Waterlogging duration (weeks)")

# E3
dia_e3_plot <- growthSlopePlot(e3_growth, "diameter", "treatment", e3_dia_emm, facet = "genotype")

# E4
dia_e4_plot <- growthSlopePlot(e4_growth, "diameter", "treatment", e4_dia_emm)


# Height
# E1
height_e1_t2_plot <- growthSlopePlot(e1_growth, "height", "duration", e1_height_T2_emm, xlab = "Waterlogging duration (weeks)")
height_e1_t3_plot <- growthSlopePlot(e1_growth, "height", "duration", e1_height_T3_emm, "T3", xlab = "Waterlogging duration (weeks)")

# E2
height_e2_t2_plot <- growthSlopePlot(e2_growth, "height", "duration", e2_height_T2_emm, xlab = "Waterlogging duration (weeks)")
height_e2_t3_plot <- growthSlopePlot(e2_growth, "height", "duration", e2_height_T3_emm, "T3", xlab = "Waterlogging duration (weeks)")

# E3
height_e3_plot <- growthSlopePlot(e3_growth, "height", "treatment", e3_height_emm, facet = "genotype")

# E4
height_e4_plot <- extPlot(e4_ext, "treatment", e4_ext_emm)


# Combined figures
ggarrange(
  ggarrange(dia_e1_t2_plot, dia_e1_t3_plot, labels = c("A", "B")),
  ggarrange(dia_e2_t2_plot, dia_e2_t3_plot, labels = c("C", "D")),
  dia_e3_plot, dia_e4_plot, ncol = 1, labels = c("", "", "E", "F")
)
ggsave(filename = "diameter_2.png", path = figures_dir, height = 20, width = 15, units = "cm")

ggarrange(
  ggarrange(height_e1_t2_plot, height_e1_t3_plot, labels = c("A", "B")),
  ggarrange(height_e2_t2_plot, height_e2_t3_plot, labels = c("C", "D")),
  height_e3_plot, height_e4_plot, ncol = 1, labels = c("", "", "E", "F")
)
ggsave(filename = "height_2.png", path = figures_dir, height = 20, width = 15, units = "cm")
...
cankerSummary <- function(data) {
  cols <- colnames(data)
  id_cols <- intersect(cols, c("block", "plot", "tree", "treatment", "duration"))
  inoc_cols <- intersect(cols, c("inoc_points", "rasp_inoculations", "leaf_scar_inoculations"))
  canker_cols <- intersect(cols, c("cankers", "inoc_cankers",  "rasp_cankers", "leaf_scar_cankers"))
  long <- melt(
    data,
    id.vars = c(id_cols, "timepoint"),
    measure.vars = canker_cols,
    variable.name = "canker_type",
    value.name = "count"
  )
  wide <- dcast(
    long,
    formula = paste(paste(id_cols, collapse = " + "), "~ canker_type + timepoint"),
    value.var = "count"
  )
  wide
  select_cols <- c(id_cols, inoc_cols)
  d <- data[timepoint == "T2", ..select_cols]
  d <- d[complete.cases(d[, ..inoc_cols])]
  result <- merge(d, wide, by = id_cols, all.x = TRUE)
  # Remove columns with all NA values
  # result <- result[, which(!sapply(.SD, function(x) all(is.na(x)))), .SDcols = names(result), with = FALSE]
  cols_to_keep <- names(result)[!sapply(result, function(x) all(is.na(x)))]
  result <- result[, ..cols_to_keep]
  canker_cols_wide <- grep("inoc|cankers", names(result), value = TRUE)
  id_cols_wide <- id_cols[id_cols != "tree"]
  # Summarise per plot
  result[, c(lapply(.SD, sum, na.rm = TRUE), .(n_trees = .N)), by = id_cols_wide, .SDcols = canker_cols_wide]
}

e1_canker <- cankerSummary(e1)
e2_canker <- cankerSummary(e2)

e1_canker[, cankers_T2_per_tree := cankers_T2 / n_trees]
e1_canker[, cankers_T3_per_tree := cankers_T3 / n_trees]

# GLM with offset
# Poisson overdispersed so use negative binomial instead
# E1 canker counts T2
design <- cankers_T2 ~ block + duration + offset(log(n_trees))
model <- glm.nb(design, data = e1_canker)

# summary(model)
# check_model(model) # good
# testResiduals(model) # good

Anova(model, type = 3)
emmeans(model, trt.vs.ctrl ~ duration, type = "response")
e1_canker_t2_emm <- setDT(as.data.frame(emmeans(model, ~ duration, type = "response")))

# T3
design <- cankers_T3 ~ block + duration + offset(log(n_trees))
model <- glm.nb(design, data = e1_canker)
...
Anova(model, type = 3)
emmeans(model, trt.vs.ctrl ~ duration, type = "response")
e1_canker_t3_emm <- setDT(as.data.frame(emmeans(model, ~ duration, type = "response")))
...
# Tweedie
# design <- (cankers_T2 / n_trees) ~ block + duration
# design <- cankers_T2_per_tree + 0.001 ~ block + duration
design <- cankers_T2_per_tree ~ block + duration

# model <- glm(design, data = e1_canker, family = tweedie(var.power = 1.5, link.power = 0))
model <- glmmTMB(design, data = e1_canker, family = glmmTMB::tweedie(link = "log"))
# model <- glm(design, data = e1_canker, family = Gamma(link = "log"))
# # model <- glm.nb(design, data = e1_canker)
# model <- glmmTMB(design, data = e1_canker, ziformula = ~1, family = gaussian(link = "log"))

# check_model(model) # reasonable
# testResiduals(model) # mild overdispersion
...
# E1 canker counts T3
# design <- cankers_T3_per_tree + 0.001 ~ block + duration
design <- cankers_T3_per_tree ~ block + duration
...
# Model
# Binomial overdispersed so use betabinomial
design <- cbind(rasp_cankers_T2, rasp_inoculations - rasp_cankers_T2) ~ block + duration
model <- glmmTMB(design, data = e2_canker, family = betabinomial(link = "logit"))
...
e2_rasp_canker_t2_emm <- setDT(as.data.frame(emmeans(model, ~ duration, type = "response")))


# E2 rasp T3
design <- cbind(rasp_cankers_T3, rasp_inoculations - rasp_cankers_T3) ~ block + duration
model <- glmmTMB(design, data = e2_canker, family = betabinomial(link = "logit"))
...
e2_rasp_canker_t3_emm <- setDT(as.data.frame(emmeans(model, ~ duration, type = "response")))
...
design <- cbind(leaf_scar_cankers_T3, leaf_scar_inoculations - leaf_scar_cankers_T3) ~ block + duration
model <- glm(design, data = e2_canker, family = binomial(link = "logit"))#, method = brglmFit)
...
e2_ls_canker_t3_emm <- setDT(as.data.frame(emmeans(model, ~ duration, type = "response")))
...
emmCankerCountPlot <- function(data, x, y, emm, facet = NULL) {
  p <- ggplot() +
    geom_jitter(
      data = data,
      aes(x = get(x), y = get(y)),# colour = Treatments),
    ...
    geom_pointrange(
      data = emm,
      aes(x = get(x), y = response, ymin = asymp.LCL, ymax = asymp.UCL),# colour = Treatments),
      position = position_dodge(width = 0.6),
      fatten = 1.8, size = 1
    ) +
    labs(x = "Waterlogging duration (weeks)", y = "Total cankers") +
    theme_bw()
  if (!is.null(facet)) p <- p + facet_wrap(facet)
  p
}
# emmCankerCountPlot(e1_canker_per_tree, "duration", "cankers_per_tree", e1_canker_emm, facet = "Treatments")
# emmCankerCountPlot(e1_canker, "duration", "cankers_T2_per_tree", e1_canker_t2_emm)
# emmCankerCountPlot(e1_canker, "duration", "cankers_T3_per_tree", e1_canker_t3_emm)

e1_canker_long <- melt(
  e1_canker, id.vars = c("block", "plot", "duration"),
  measure.vars = c("cankers_T2", "cankers_T3"),
  variable.name = "Treatments", value.name = "cankers"
)
e1_canker_long[, Treatments := fcase(
  Treatments == "cankers_T2", "single",
  Treatments == "cankers_T3", "repeat"
)][, Treatments := factor(Treatments, levels = c("single", "repeat"))]
e1_canker_emm <- bind_rows(
  "single" = e1_canker_t2_emm, "repeat" = e1_canker_t3_emm, .id = "Treatments"
)[, Treatments := factor(Treatments, levels = c("single", "repeat"))]

e1_canker_plot <- emmCankerCountPlot(e1_canker_long, "duration", "cankers", e1_canker_emm, "Treatments")


emmCankerPlotColour <- function(data, x, emm, xlab, colour, facet = NULL) {
  p <- ggplot() +
    geom_jitter(
      data = data,
      aes(x = get(x), y = cankers / inoc, colour = get(colour)),
      position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.6),
      alpha = 0.2, size = 2
    ) +
    geom_pointrange(
      data = emm,
      aes(x = get(x), y = prob, ymin = asymp.LCL, ymax = asymp.UCL, colour = get(colour)),
      position = position_dodge(width = 0.6),
      fatten = 1.8, size = 1
    ) +
    scale_y_continuous(labels = label_percent(accuracy = 1), limits = c(0, 1)) +
    labs(x = xlab, y = "Canker incidence") +
    theme_bw()
  if (!is.null(facet)) {
...
# emmCankerPlotColour(e2_canker_long, "duration", e2_canker_emm, "Waterlogging duration (weeks)", "Treatments", facet = "wound")
...
e2_canker_long <- melt(
  e2_canker, id.vars = c("block", "plot", "duration"),
  measure.vars = c("rasp_cankers_T2", "rasp_cankers_T3", "leaf_scar_cankers_T2", "leaf_scar_cankers_T3"),
  variable.name = "src", value.name = "cankers"
)
e2_canker_long[, c("wound_raw", "tp") := tstrsplit(src, "_cankers_", fixed = TRUE)]
e2_canker_long[, wound := fcase(
  wound_raw == "rasp", "Rasp",
  wound_raw == "leaf_scar", "Leaf scar"
)]
e2_canker_long[, Treatments := fcase(
  tp == "T2", "single",
  tp == "T3", "repeat"
)][, Treatments := factor(Treatments, levels = c("single", "repeat"))]
e2_canker_long[, c("src", "wound_raw", "tp") := NULL]

inoc <- melt(
  e2_canker, id.vars = c("block", "plot", "duration"),
  measure.vars = c("rasp_inoculations", "leaf_scar_inoculations"),
  variable.name = "src", value.name = "inoc"
)[, wound := fcase(
  src == "rasp_inoculations", "Rasp",
  src == "leaf_scar_inoculations", "Leaf scar"
)][, src := NULL]

e2_canker_long <- merge(
  e2_canker_long, inoc,
  by = c("block", "plot", "duration", "wound"),
  all.x = TRUE
)

e2_canker_emm <- bind_rows(
  e2_rasp_canker_t2_emm[, `:=`(Treatments = "single", wound = "Rasp")],
  e2_rasp_canker_t3_emm[, `:=`(Treatments = "repeat", wound = "Rasp")],
  e2_ls_canker_t3_emm[, `:=`(Treatments = "repeat", wound = "Leaf scar")],
  data.table(
    duration = c("0", "1", "2", "4", "8"),
    prob = NA, SE = NA, df = NA, "asymp.LCL" = NA, "asymp.UCL" = NA,
    Treatments = "single", wound = "Leaf scar"
  )
)[, Treatments := factor(Treatments, levels = c("single", "repeat"))]

e2_canker_plot <- emmCankerPlot(e2_canker_long, "duration", e2_canker_emm, "Waterlogging duration (weeks)", facet = c("wound", "Treatments"))

ggarrange(e1_canker_plot, e2_canker_plot, labels = c("A", "B"), ncol = 1, heights = c(1, 2))
ggsave(filename = "canker_E1_E2.png", path = figures_dir, width = 14, height = 16, units = "cm")


e3_canker_plot <- emmCankerPlot(e3_canker, "treatment", e3_canker_emm, "Waterlogging treatment", facet = c("genotype", "type"))
e4_canker_plot <- emmCankerPlot(e4_canker, "treatment", e4_canker_emm, "Waterlogging treatment", facet = "type") +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))

ggarrange(e3_canker_plot, e4_canker_plot, labels = c("A", "B"), ncol = 1, heights = c(2, 1))
ggsave(filename = "canker_E3_E4.png", path = figures_dir, width = 14, height = 16, units = "cm")
...
# ggarrange(
#   e1_canker_plot, e2_canker_plot, c, d,
#   labels = c("A", "B", "C", "D"), vjust = 0.8,
#   nrow = 4, ncol = 1, heights = c(1, 2, 2, 1)#, align = "hv"
# )
