...
# Model designs
E1_DESIGN <- ~ treatment * timepoint + (1 | block) + (1 | tree) + (1 | pot)
E2_DESIGN <- ~ treatment * timepoint + (1 | block) + (1 | tree) + (1 | pot)
E3_DESIGN <- ~ treatment * (timepoint + genotype) + (1 | block / splitPlot) + (1 | tree)
# E4_DESIGN

E1_E2_DESIGN <- ~ treatment * tmiepoint * experiment + (1 | experiment:block) + (1 | experiment:tree) + (1 | experiment:pot)
...
e2[, diameter := coalesce(diameter.x, diameter.y)][, c("diameter.x", "diameter.y") := NULL]
...
design <- y ~ duration * timepoint + (1 | block) + (1 | tree) + (1 | pot)

# E1 diameter model

e1_diameter_model <- lmer(update(E1_DESIGN, diameter ~ .), data = e1)
...
e2_diameter_model <- lmer(update(E2_DESIGN, diameter ~ .), data = e2)
...
emm_height_e1_e2 <- emmeans(e1_e2_diameter_model, trt.vs.ctrl ~ duration | experiment | timepoint)
...
e1_height_model <- lmer(
  ...
  update(E1_DESIGN, height ~ .),
  data = e1[!(health %in% top_removals)]
)
...
emm_height <- emmeans(e2_height_model, trt.vs.ctrl ~ duration | timepoint)
