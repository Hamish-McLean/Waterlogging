...
# Model designs
E1_DESIGN <- ~ duration * timepoint + (1 | block) + (1 | tree) + (1 | pot)
E2_DESIGN <- ~ duration * timepoint + (1 | block) + (1 | tree) + (1 | pot)
E3_DESIGN <- ~ treatment * (timepoint + genotype) + (1 | block / splitPlot) + (1 | tree)
E4_DESIGN <- ~ treatment * timepoint + (1 | block) + (1 | tree)

E1_E2_DESIGN <- ~ duration * (timepoint + experiment) + (1 | experiment:block) + (1 | experiment:tree) + (1 | experiment:pot)
...
plot_tree_health <- function(health_summary, x, facet_rows = NULL) {
  ...
  if (!is.null(facet_rows)) {
    p <- p + facet_wrap(vars(!!sym(facet_rows)), ncol = 1)
  }
...
plot_tree_health(e3_health_summary[timepoint == "T1"], "treatment", "genotype")
...
e3_rasp_canker_model <- glmer(
  cbind(rasp_cankers, rasp_inoculations - rasp_cankers) ~ treatment * genotype + (1 | block / splitPlot) + (1 | inoculation_group),
  family = binomial(link = "logit"),
...
e3_leaf_scar_canker_model <- glmer(
  cbind(leaf_scar_cankers, leaf_scar_inoculations - leaf_scar_cankers) ~ treatment * genotype + (1 | block / splitPlot) + (1 | inoculation_group),
  family = binomial(link = "logit"),
