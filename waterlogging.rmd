...
emmMortalityPlot(e1_e2_mortality, "duration", e1_e2_mortality_emm, "experiment")
ggsave(filename = "mortality_E1_E2.png", path = figures_dir)


# E3

emmMortalityPlot(e3_mortality, "treatment", e3_mortality_emm, "genotype")
...
e2_rasp_canker <- e2[
  ...
  .(rasp_cankers = sum(rasp_cankers), rasp_inoc = sum(rasp_inoculations)),
  by = .(plot, block, duration)
]

# Model
design <- cbind(rasp_cankers, rasp_inoc - rasp_cankers) ~ block + duration
...
# Model
design <- cbind(ls_cankers, ls_inoc - ls_cankers) ~ block + duration
model <- glm(design, data = e2_ls_canker, family = binomial(link = "logit"), method = brglmFit)
check_model(model) # poor
sim <- simulate_residuals(model)
testResiduals(sim) # significant dispersion
testZeroInflation(sim)
...
# betabinomial
design <- cbind(ls_cankers, ls_inoc - ls_cankers) ~ block + duration
model <- glmmTMB(design, data = e2_ls_canker, family = betabinomial(link = "logit"))
check_model(model) # 
sim <- simulate_residuals(model)
testResiduals(sim) #
testZeroInflation(sim) #
...
design <- cbind(rasp_cankers, rasp_inoc - rasp_cankers) ~ genotype * treatment + (1 | block / genotype)
model <- glm(design, data = e3_rasp_canker, family = binomial(link = "logit"), method = brglmFit)
check_model(model) # poor
sim <- simulate_residuals(model)
testResiduals(sim) # significant dispersion
testZeroInflation(sim)
