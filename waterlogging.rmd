...
growthSlopePlot <- function(
  ...
  p <- ggplot() +
    geom_segment(
      ...
      size = 0.4, alpha = 0.5#.2
    ) +
    geom_point(data = dat, aes(x = x_T1, y = T1, colour = block), alpha = 0.5) +
    geom_point(data = dat, aes(x = x_T2, y = T2, colour = block), alpha = 0.5) +
    geom_pointrange(
      data = emm,
...
extPlot <- function(data, x, emm) {
  ...
  geom_jitter(
    ...
    width = 0.3, height = 0, alpha = 0.5, size = 2
  ) +
  geom_pointrange(
    data = emm,
...
# check_model(model) # fine
# testResiduals(model) # a bit overdispersed
...
emmCankerCountPlot <- function(data, x, y, emm, facet = NULL) {
  p <- ggplot() +
    ...
    labs(x = "Waterlogging duration (weeks)", y = "Cankers per tree") +
    theme_bw()
...
e1_canker_per_tree <- melt(
  e1_canker, id.vars = c("block", "plot", "duration"), 
  measure.vars = c("cankers_T2_per_tree", "cankers_T3_per_tree"),
  variable.name = "Treatments", value.name = "cankers_per_tree"
)
e1_canker_per_tree[, Treatments := fcase(
  Treatments == "cankers_T2_per_tree", "single",
  Treatments == "cankers_T3_per_tree", "repeat"
...
e1_canker_plot <- emmCankerCountPlot(e1_canker_per_tree, "duration", "cankers_per_tree", e1_canker_emm, "Treatments")


emmCankerPlotColour <- function(data, x, emm, xlab, facet = NULL) {
  p <- ggplot() +
    geom_jitter(
      data = data,
