...
- Plot mortality of each block per treatment group with error bars
...
e1_health <- e1[timepoint == "T2" & !is.na(health)][, health := fcase(
  health %in% healthy, "healthy", health == "dead", "dead"
) |> as.factor()]


design <- health ~ duration + (1 | block / plot / pot)
model <- glmmTMB(design, e1_health, family = betabinomial(link = "logit"))
check_model(model)
# Anova(model, type = 3)
anova(update(model, . ~ (1 | block / plot / pot)), model)
emmeans(model, trt.vs.ctrl ~ duration)

m1 <- glmer(health ~ (1 | block / plot / pot), e1_health, family = binomial(link = "logit"))
check_model(m1)
anova(m1, model)
...
new approach:
summarise by plot
Use brglm2 bias reduction glm
Or use betabinomiaol if overdispersed
But how to handle split plot in E3?
...
model <- glm(
  cbind(dead, total - dead) ~ experiment:block + experiment * duration,
  data   = mortalityByPlot(rbind(e1, e2, fill = TRUE)),
  family = binomial(link = "logit"),
  method = brglmFit
)
# check_model(model) # fine
...
anova(update(model, . ~ experiment/block + experiment:duration), model, test = "Chisq")
anova(update(model, . ~ experiment:block + duration), model, test = "Chisq")
anova(update(model, . ~ experiment/block + duration), model, test = "Chisq")
...
# model <- glmer(design, data = mortalityByPlotE3(e3), family = binomial(link = "logit"))
# check_model(model) # bad

model <- glmmTMB(design, data = mortalityByPlotE3(e3), family = betabinomial(link = "logit"))
check_model(model)
...
# E2 canker model
# design <- cbind(rasp_cankers, rasp_inoculations - rasp_cankers) ~ duration + (1 | block) + (1 | inoculation),
design <- cbind(rasp_cankers, rasp_inoculations - rasp_cankers) ~ duration + (1 | block / plot / pot) + (1 | inoculation)
...
design <- cbind(rasp_cankers, rasp_inoculations - rasp_cankers) ~ genotype * treatment + (1 | block / genotype / plot) + (1 | inoculation_group)
...
emmeans(e3_leaf_scar_canker_model, pairwise ~ genotype, type = "response") |> confint()
